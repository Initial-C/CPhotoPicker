...
@interface CPhotoDataManager : NSObject
 ...
 *  相册权限状态
 */
+ (void)isPhotoLibraryVisible:(void(^)(PhotoAuthorizationStatus status))authorazitionStatus;

+ (void)requestAuthorization:(void (^)(PhotoAuthorizationStatus status))authorizationStatus;

/**
 *  相机权限状态
 */
+ (void)isCameraVisible:(void (^)(AVAuthorizationStatus status))authorazitionStatus;
